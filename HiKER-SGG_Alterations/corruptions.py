...
def _get_asset_root():
    """Return the path to HiKER-SGG_Alterations folder (where this script is located)."""
...
def waterdrop(x, severity=1):
    ...
    root_path = _get_asset_root() + os.sep
    noise = PILImage.open(root_path + 'PythonShaders' + os.sep + 'noise.png')
    ...
    # Setup Shader:
    shader = Shadertoy.create_from_file(window.get_size(), root_path + 'PythonShaders' + os.sep + 'shaders' + os.sep + c)
